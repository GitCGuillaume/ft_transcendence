...
export class Matchmaker<P> {
  ...
  constructor(
    ...
    this.maxMatchSize =
      (options &&
        options.maxMatchSize &&
        options.maxMatchSize > 0 &&
        options.maxMatchSize) ||
      2;
    this.minMatchSize =
      (options &&
        options.minMatchSize &&
        options.minMatchSize > 0 &&
        options.minMatchSize) ||
      this.maxMatchSize;
  }
  ...
  public getPlayerState(player: P): IPlayerState {
    ...
    if (
      this.queue.find((queuePlayer: P) => playerKey == this.getKey(queuePlayer))
    ) {
      return IPlayerState.INQUEUE;
    } else if (
      this.inGame.find((game: Game<P>) => {
        return game.players.find(
          (gamePlayer: P) => playerKey == this.getKey(gamePlayer)
        );
      })
    ) {
  ...
  public leaveQueue(player: P): void {
    ...
    if (index == -1) throw Error(errorMessages.playerNotInQueue);
  ...
  public endGame(players: P | P[]): void {
    ...
    for (let player of players) {
      let playerKey = this.getKey(player);
      gameIndex = this.inGame.findIndex((game: Game<P>) => {
        return game.players.find(
          (gamePlayer: P) => playerKey == this.getKey(gamePlayer)
        );
      });
    ...
    if (gameIndex == -1) throw Error(errorMessages.gameDoesNotExists);
  ...
  public indexOnQueue = (player: P): number => {
    let playerKey = this.getKey(player);
    let index;
    index = this.queue.findIndex((player) => {
      return this.getKey(player) == playerKey;
    });
    return index;
  };
}
